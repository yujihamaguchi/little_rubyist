...
class SliderSwitchE2eTest < Minitest::Test
  DEVICE_CLASSES = [Fan, RecordPlayer].freeze
  ...
  def test_slider_down_device
    DEVICE_CLASSES.each do |device_class|
      ...
      switch.turn_on
      assert_equal 100, device.output_level

      # Act
      switch.slider_down(by: 20)

      # Assert
      assert_equal 80, device.output_level
    end
  end
  ...
  def test_slider_up_device
    DEVICE_CLASSES.each do |device_class|
      ...
      assert_equal 70, device.output_level

      # Act
      switch.slider_up(by: 20)

      # Assert
      assert_equal 90, device.output_level
    end
  end
end
